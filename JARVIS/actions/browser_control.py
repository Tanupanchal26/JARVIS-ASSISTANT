    ...
    async def type_text(self, selector: str = None, text: str = "",
        ...
        try:
            ...
            if selector:
                ...
                # No selector given – fall back to focused element or a textarea
                el = page.locator(":focus").first if page.locator(":focus").first else page.locator('textarea').first
            if clear_first:
                await el.clear()
            await el.type(text, delay=50)
            return "Text typed."
        except Exception as e:
            # Fallback: try any visible textarea on the page
            try:
                el = page.locator('textarea').first
                if clear_first:
                    await el.clear()
                await el.type(text, delay=50)
                return "Text typed (fallback)."
            except Exception:
